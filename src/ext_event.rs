...
use crate::{app::win_handler::EXT_EVENT_IDLE_TOKEN, id::ExtHandleId};
use crate::{id::WindowId, shell::IdleHandle};

pub(crate) type ExtCommand = (ExtHandleId, Box<dyn Any + Send>);
...
impl ExtEventHost {
    ...
    pub(crate) fn make_sink(&self, ext_handle_id: ExtHandleId) -> ExtEventSink {
        ExtEventSink {
    ...
    pub(crate) fn recv(&mut self) -> Option<ExtCommand> {
...
impl ExtEventSink {
    ...
    pub fn submit_command<T: Any + Send>(
        ...
        self.queue
            ...
            .push_back((self.ext_handle_id, payload));
        Ok(())
    }
}
